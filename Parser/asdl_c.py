...
# Not a complete list, just the ones that match an AST field
c_keywords = {"default"}
...
class StructVisitor(EmitVisitor):
    ...
    def sum_with_constructors(self, sum, name, depth):
        ...
        for field in sum.attributes:
            ...
            emit("%s %s;" % (type, get_c_field_name(field.name)), depth + 1);
        emit("};")
        emit("")
    ...
    def visitField(self, field, depth):
        ...
        name = get_c_field_name(field.name)
        if field.seq:
    ...
    def visitProduct(self, product, name, depth):
        ...
        for field in product.attributes:
            ...
            type = str(field.type)
            assert type in asdl.builtin_types, type
            self.emit("%s %s;" % (type, get_c_field_name(field.name)), depth + 1);
        self.emit("};", depth)
        self.emit("", depth)
...
class PrototypeVisitor(EmitVisitor):
    ...
    def emit_function(self, name, ctype, args, attrs, union=True):
        args = args + attrs
        if args:
            argstr = ", ".join(["%s %s" % (atype, get_c_field_name(aname))
...
class FunctionVisitor(PrototypeVisitor):
    ...
    def emit_body_union(self, name, args, attrs):
        ...
        for argtype, argname, opt in args:
            argname = get_c_field_name(argname)
...
class Obj2ModVisitor(PickleVisitor):
    ...
    def buildArgs(self, fields):
        fields = [get_c_field_name(field) for field in fields]
    ...
    def visitFieldDeclaration(self, field, name, sum=None, prod=None, depth=0):
        ...
        if field.seq:
            if self.isSimpleType(field):
                self.emit("asdl_int_seq* %s;" % get_c_field_name(field.name), depth)
            else:
                _type = field.type
                self.emit(f"asdl_{field.type}_seq* {get_c_field_name(field.name)};", depth)
        else:
            ctype = get_c_type(field.type)
            self.emit("%s %s;" % (ctype, get_c_field_name(field.name)), depth)
    ...
    def visitField(self, field, name, sum=None, prod=None, depth=0):
        ...
        self.emit(line % get_c_field_name(field.name), depth)
            ...
            if not field.opt:
                ...
                if self.isNumeric(field):
                    if field.name in self.attribute_special_defaults:
                        self.emit(
                            "%s = %s;" % (get_c_field_name(field.name), self.attribute_special_defaults[field.name]),
                            depth+1,
                        )
                    else:
                        self.emit("%s = 0;" % get_c_field_name(field.name), depth+1)
                elif not self.isSimpleType(field):
                    self.emit("%s = NULL;" % get_c_field_name(field.name), depth+1)
                else:
        ...
        if field.seq:
            ...
            if self.isSimpleType(field):
                self.emit("%s = _Py_asdl_int_seq_new(len, arena);" % get_c_field_name(field.name), depth+1)
            else:
                self.emit("%s = _Py_asdl_%s_seq_new(len, arena);" % (get_c_field_name(field.name), field.type), depth+1)
            self.emit("if (%s == NULL) goto failed;" % get_c_field_name(field.name), depth+1)
            ...
            self.emit("}", depth+2)
            self.emit("asdl_seq_SET(%s, i, val);" % get_c_field_name(field.name), depth+2)
            self.emit("}", depth+1)
        else:
            with self.recursive_call(name, depth+1):
                self.emit("res = obj2ast_%s(state, tmp, &%s, arena);" %
                          (field.type, get_c_field_name(field.name)), depth+1)
        ...
        for field in fields:
            if field.opt:
                self.emit('if (PyObject_SetAttr(state->%s_type, state->%s, Py_None) == -1)' %
                            (name, get_c_field_name(field.name)), depth)
                self.emit("return -1;", depth+1)
...
class ObjVisitor(PickleVisitor):
    ...
    def visitField(self, field, name, depth, product):
        def emit(s, d):
            self.emit(s, depth + d)
        if product:
            value = "o->%s" % get_c_field_name(field.name)
        else:
            value = "o->v.%s.%s" % (name, get_c_field_name(field.name))
        self.set(field, value, depth)
        emit("if (!value) goto failed;", 0)
        emit("if (PyObject_SetAttr(result, state->%s, value) == -1)" % get_c_field_name(field.name), 0)
        emit("goto failed;", 1)
...
def generate_module_def(mod, metadata, f, internal_h):
    ...
    state_strings = {
        ...
        "__module__",
        "_attributes",
        *[get_c_field_name(name) for name in metadata.identifiers]
    }
